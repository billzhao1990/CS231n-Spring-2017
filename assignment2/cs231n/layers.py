...
def affine_backward(dout, cache):
    ...
    dx = dout.dot(w.T).reshape(N, *D_tuple)
...
def batchnorm_backward(dout, cache):
    ...
    dout_dx_hat = dout * gamma
    dout_dvar = np.sum(dout_dx_hat * (-0.5) * (x - sample_mean) * ((sample_var + eps)**(-1.5)), axis=0)
    dout_dmean = np.sum(-dout_dx_hat / np.sqrt(sample_var+eps), axis=0)
    
    dmean_dx = 1./N * np.ones(x.shape)
    dvar_dx = 1./N * (\
                      2 * (x - sample_mean) \
                      - 2 * np.sum((x - sample_mean) * dmean_dx, axis=0)\
                     ) 
    
    dx = dout_dx_hat*1./np.sqrt(sample_var + eps)\
        + dout_dmean * dmean_dx\
        + dout_dvar * dvar_dx
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1. / N) * ((sample_var + eps)**(-1/2)) * (N*dxhat - np.sum(dxhat, axis=0) - x_hat*np.sum(dxhat*x_hat, axis=0))
    dbeta = np.sum(dout, axis=0)
    ...
    for n in range(N):
        ...
        for input_top in range(0, H + 2*pad - HH + 1, stride):
            ...
            for input_left in range(0, W+2*pad-WW+1, stride):
                ...
                for f in range(F):
                    weight = w[f,:,:,:]
                    out[n, f, out_h_target, out_w_target] = np.sum(input_x * weight) + b[f]      
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        ...
        dx_sample_pad = np.pad(dx[n, :, :, :], pad_width=[(0,0),(pad,pad),(pad,pad)], mode='constant')
